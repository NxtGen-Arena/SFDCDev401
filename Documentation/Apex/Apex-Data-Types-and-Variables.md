...
* Class Name: Use PascalCase (e.g., ClassNamePOSTFIX). Class names should be nouns, unique, and avoid spaces or underscores.
* Variable Name: Use camelCase (e.g., accountList). The first letter is lowercase, and subsequent words are capitalized.
* Method Name: Use camelCase with a verb-first approach (e.g., showAccountDetail()). The first letter is lowercase, and subsequent words are capitalized.
* Constants: Use all uppercase with underscores to separate words (e.g., ACCOUNT_LIMIT). 
